...
noremap <unique> <c-F2> :MarksBrowser<enter>
noremap <unique> <script> \sm m
"noremap <silent> m :exe 'norm \sm'.nr2char(getchar())<bar><enter>
noremap <silent> m :call <sid>NewMark()<enter>
"au BufWinEnter *.* :call <sid>RefreshMarksWin()
"noremap <silent> m :exe 'norm \sm'.nr2char(getchar())<bar>ca

fun! s:RefreshMarksWin()
    if s:isShown == 1
        let winNo = bufwinnr(s:bufNo)
        call s:_ShowMarksWin(winNo)
        ":wincmd w
    endif
endf

fun! s:NewMark()
    execute 'norm \sm'.nr2char(getchar())
    if s:isShown == 1
        call s:RefreshMarksWin()
        :wincmd w
    endif
endf
