...
Context types
  It tests escape string.
    ShouldEqual neocomplcache#filters#fuzzy_escape(
          \ 'abc'), 'a.*b.*c.*'
    ShouldEqual neocomplcache#filters#fuzzy_escape(
          \ '%a%b%c'), '%%a.*%%b.*%%c.*'
    ShouldEqual neocomplcache#filters#fuzzy_escape(
  ...
  It tests sort functions.
    ...
    let start = reltime()
    call neocomplcache#filters#sorter_rank#define().filter(
          \   {'candidates' : copy(candidates), 'input' : '' })
    echomsg reltimestr(reltime(start))

    ShouldEqual sort(copy(candidates), 'CompareRank'),
          \ neocomplcache#filters#sorter_rank#define().filter(
          \   {'candidates' : copy(candidates), 'input' : '' })
  End

  It tests fuzzy matcher.
    ShouldEqual neocomplcache#filters#matcher_fuzzy#define().filter(
